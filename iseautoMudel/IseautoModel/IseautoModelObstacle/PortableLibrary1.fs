namespace IseautoModelObstacle
...
type iseautoTest()=
 ...
 //Sisse loetavate teekonnapunktide fail
 static let waypointsFilePath="../wp.csv"
 static let temporalWaypointsPath="../wp.csv"
 ...
 static let _temporalWaypointData = CsvProvider<Schema = "x (float), y (float), z (float), yaw (float),velocity(float),change_flag(float)", HasHeaders=false>.Load(waypointsFilePath)
 ...
 static member StartPosition([<Domain("CoordinatesX")>] xcoord:string,[<Domain("CoordinatesY")>] xcoord2:string,[<Domain("CoordinatesZ")>] ycoord2:string,[<Domain("Velocity")>] zcoord2:string,[<Domain("Yaw")>] yaw:string,[<Domain("CountOfWp")>]wp:int32)=
   ...
   iseautoTest.numOfWpPubAtOnce<-15
 ...
 [<Action>]
  static member MoveCar([<Domain("CoordinatesX")>] xcoord:string,[<Domain("CoordinatesY")>] xcoord2:string,[<Domain("CoordinatesZ")>] ycoord2:string,[<Domain("Velocity")>] zcoord2:string,[<Domain("Yaw")>] yaw:string,[<Domain("CountOfWp")>]wp:int32,[<Domain("TempCoordinatesX")>] tx:string,[<Domain("TempCoordinatesY")>] ty:string,[<Domain("TempCoordinatesZ")>] tz:string,[<Domain("TempYaw")>] tyaw:string,[<Domain("TempVelocity")>] tvel:string)=
   iseautoTest.stepcount<-iseautoTest.stepcount+iseautoTest.numOfWpPubAtOnce
   ...
   static member MoveCarEnabled()=( iseautoTest.transition=Transitions.Move && iseautoTest.positionState=PositionState.ReacheWp  && iseautoTest.stepcount<=waypointCount-1)
    ...
    static member TempCoordinatesX () : NModel.Set<string> =
        let waypointData2 =CsvProvider<Schema="x (float), y (float), z (float), yaw (float), velocity (float), change_flag (float)",HasHeaders=false>.Load(temporalWaypointsPath) 
     ...
     static member TempCoordinatesY () : NModel.Set<string> =
        let waypointData2 =CsvProvider<Schema="x (float), y (float), z (float), yaw (float), velocity (float), change_flag (float)",HasHeaders=false>.Load(temporalWaypointsPath) 
        let aSequence =waypointData2.Rows|>Seq.skip (iseautoTest.stepcount) |>Seq.take  (if iseautoTest.stepcount+iseautoTest.numOfWpPubAtOnce-1<waypointCount then iseautoTest.numOfWpPubAtOnce else if iseautoTest.stepcount>waypointCount then 0  else waypointCount-iseautoTest.stepcount )
       ...
       static member TempCoordinatesZ () : NModel.Set<string> =
        let waypointData2 =CsvProvider<Schema="x (float), y (float), z (float), yaw (float), velocity (float), change_flag (float)",HasHeaders=false>.Load(temporalWaypointsPath) 
        let aSequence =waypointData2.Rows|>Seq.skip (iseautoTest.stepcount) |>Seq.take  (if iseautoTest.stepcount+iseautoTest.numOfWpPubAtOnce-1<waypointCount then iseautoTest.numOfWpPubAtOnce else if iseautoTest.stepcount>waypointCount then 0  else waypointCount-iseautoTest.stepcount )
        ...
        if result.Length>0 then
          ...
          static member TempYaw () : NModel.Set<string> =
        let waypointData2 =CsvProvider<Schema="x (float), y (float), z (float), yaw (float), velocity (float), change_flag (float)",HasHeaders=false>.Load(temporalWaypointsPath) 
        let aSequence =waypointData2.Rows|>Seq.skip (iseautoTest.stepcount) |>Seq.take  (if iseautoTest.stepcount+iseautoTest.numOfWpPubAtOnce-1<waypointCount then iseautoTest.numOfWpPubAtOnce else if iseautoTest.stepcount>waypointCount then 0  else waypointCount-iseautoTest.stepcount )
        ...
        if result.Length>0 then
         ...
         static member TempVelocity () : NModel.Set<string> =
        let waypointData2 =CsvProvider<Schema="x (float), y (float), z (float), yaw (float), velocity (float), change_flag (float)",HasHeaders=false>.Load(temporalWaypointsPath) 
        let aSequence =waypointData2.Rows|>Seq.skip (iseautoTest.stepcount) |>Seq.take  (if iseautoTest.stepcount+iseautoTest.numOfWpPubAtOnce-1<waypointCount then iseautoTest.numOfWpPubAtOnce else if iseautoTest.stepcount>waypointCount then 0  else waypointCount-iseautoTest.stepcount )
  ...
  //Asünkroone toiming auto teekonna jõudmise kontrollimise jaoks  
  [<Action>]
  ...
  static member didReacheWaypoint_FinishEnabled()=(iseautoTest.startedWaitingMsg && iseautoTest.transition=Transitions.ObserveMsg)
   ...
   //Toiming mis viitab edukalt läbitud teekonnale
  [<Action>]
  static member Goal()=
   iseautoTest.positionState<-PositionState.Finish
   iseautoTest.transition<-Transitions.Stop
...
type Contract()=
    static member Create()=
        LibraryModelProgram(typedefof<Contract>.Assembly,"IseautoModelObstacle")
